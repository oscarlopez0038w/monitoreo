...
export async function sendGa4RefundEvent({
  ...
  const formattedItems = (items || []).map((it) => {
    const itemPrice = typeof it.sellingPrice === 'number'
      ? it.sellingPrice
  ...
  const rawTxId = String(orderId || transactionId || `TX-${Date.now()}`);
  // Remover sufijos de seller VTEX (ej. -01, -02) para coincidir con el transaction_id que registra el píxel de VTEX en GA4
  const targetTxId = rawTxId.replace(/-\d+$/, '');
  const refundId = `REFUND-${targetTxId}`;
