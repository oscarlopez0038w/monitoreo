...
export async function verifySessionToken(token) {
  ...
  try {
    ...
    // Actualizar fecha de última actividad (last_login_at) con throttling (máximo una vez cada 3 minutos)
    const lastActiveMs = dbUser.last_login_at ? new Date(dbUser.last_login_at).getTime() : 0;
    const nowMs = Date.now();
    if (nowMs - lastActiveMs > 3 * 60 * 1000) {
      supabaseAdmin
        .from('app_users')
        .update({ last_login_at: new Date().toISOString() })
        .eq('id', dbUser.id)
