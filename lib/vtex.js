...
export async function fetchAbandonedCarts({
  ...
  // Parseo de fechas para filtrado por rango
  let startMs = null;
  let endMs = null;
  if (startDate) {
  ...
  try {
    ...
    if (isSupabaseConfigured() && emails.length > 0) {
      try {
        const [ordersRes, txsRes, notifRes] = await Promise.all([
          supabaseAdmin
            .from('vtex_orders')
            .select('order_id, client_email, client_name, creation_date, status, total_value')
            .in('client_email', emails),
          supabaseAdmin
            .from('vtex_transactions')
            .select('transaction_id, client_email, client_name, status, return_code, return_message, error_title, start_date')
            .in('client_email', emails),
          supabaseAdmin
    ...
    for (const item of initialCarts) {
      ...
      // Órdenes y transacciones del cliente
      const matchingOrders = dbOrders.filter((o) => o.client_email?.toLowerCase() === cEmail);
      const matchingTxs = dbTransactions.filter((t) => t.client_email?.toLowerCase() === cEmail);

      // Verificación de compra posterior o simultánea (conversión exitosa)
      const postOrder = matchingOrders.find((o) => {
        const oTime = new Date(o.creation_date).getTime();
        return oTime >= sessionTime - 15 * 60 * 1000;
      });
      const isConverted = !!postOrder;
      ...
      if (!finalFirstName && !finalLastName) {
        ...
        if (matchingOrders.length > 0 && matchingOrders[0].client_name) {
          const parts = matchingOrders[0].client_name.trim().split(/\s+/);
      ...
      // Contacto riguroso (revisar homePhone y businessPhone ya que phone a menudo viene vacío en CL)
      const finalPhone = c.phone || c.homePhone || c.businessPhone || '';
      const finalDocument = c.document || '';
  ...
  if (isSupabaseConfigured()) {
    try {
      ...
      if (!dbError) {
        ...
        console.warn('Aviso: No se pudo registrar en vtex_abandoned_cart_notifications (es posible que la tabla aún no exista):', dbError.message);
      }
    } catch (errDb) {
      console.warn('Excepción registrando en vtex_abandoned_cart_notifications:', errDb.message);
    }
  }
