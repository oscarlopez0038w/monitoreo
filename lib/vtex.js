...
export async function fetchSkuPrice(skuId) {
  ...
  while (retries > 0) {
    try {
      ...
      const listPrice = data.listPrice !== undefined && data.listPrice !== null ? data.listPrice : (data.fixedPrices?.[0]?.listPrice ?? null);
      const basePrice = data.basePrice !== undefined && data.basePrice !== null ? data.basePrice : (data.fixedPrices?.[0]?.value ?? data.value ?? null);
