...
export async function fetchSkuPrice(skuId) {
  ...
  while (retries > 0) {
    try {
      ...
      // Priorizar el precio de venta activo del Canal de Ventas Principal (fixedPrices[0].value)
      const listPrice =
        data.fixedPrices?.[0]?.listPrice !== undefined && data.fixedPrices?.[0]?.listPrice !== null
          ? data.fixedPrices[0].listPrice
          : data.listPrice !== undefined && data.listPrice !== null
          ? data.listPrice
          : null;

      const basePrice =
        data.fixedPrices?.[0]?.value !== undefined && data.fixedPrices?.[0]?.value !== null
          ? data.fixedPrices[0].value
