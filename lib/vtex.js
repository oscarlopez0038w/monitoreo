...
export async function fetchAbandonedCarts({
  ...
  try {
    ...
    // 4. Filtrar por período/rango de fechas para obtener los carritos exactos de este corte
    let periodCarts = processedCarts;
    if (startMs) periodCarts = periodCarts.filter((c) => c.sessionTime >= startMs);
    if (endMs) periodCarts = periodCarts.filter((c) => c.sessionTime <= endMs);

    if (!startMs && !endMs) {
      if (period === '24h') periodCarts = periodCarts.filter((c) => c.abandonedMinutesAgo <= 24 * 60);
      if (period === '7d') periodCarts = periodCarts.filter((c) => c.abandonedMinutesAgo <= 7 * 24 * 60);
      if (period === '30d') periodCarts = periodCarts.filter((c) => c.abandonedMinutesAgo <= 30 * 24 * 60);
    }

    // 5. Calcular Funnel de Etapas y Métricas Base sobre los carritos del período
    const stageCounters = {
    ...
    for (const c of periodCarts) {
      if (c.isConverted) {
        stageCounters.converted.count++;
        stageCounters.converted.totalValue += c.rawCartValue;
      } else if (stageCounters[c.stage]) {
        stageCounters[c.stage].count++;
        stageCounters[c.stage].totalValue += c.rawCartValue;
      }
    }

    const realAbandonedCount =
      stageCounters.cart.count +
      stageCounters.profile.count +
      stageCounters.shipping.count +
      stageCounters.payment.count;

    const realAbandonedTotalValue =
      stageCounters.cart.totalValue +
      stageCounters.profile.totalValue +
      stageCounters.shipping.totalValue +
      stageCounters.payment.totalValue;

    const convertedCount = stageCounters.converted.count;
    const convertedTotalValue = stageCounters.converted.totalValue;

    // 6. Filtrado de vista según parámetros del usuario (etapa, exclusión de compras, búsqueda, valor mínimo)
    let filteredCarts = periodCarts;
    ...
    // Calcular porcentajes de etapas del Funnel sobre los carritos abandonados reales del período
    const stages = {
      cart: {
        count: stageCounters.cart.count,
        totalValue: stageCounters.cart.totalValue,
        pct: realAbandonedCount > 0 ? Math.round((stageCounters.cart.count / realAbandonedCount) * 100) : 0,
      },
      profile: {
        count: stageCounters.profile.count,
        totalValue: stageCounters.profile.totalValue,
        pct: realAbandonedCount > 0 ? Math.round((stageCounters.profile.count / realAbandonedCount) * 100) : 0,
      },
      shipping: {
        count: stageCounters.shipping.count,
        totalValue: stageCounters.shipping.totalValue,
        pct: realAbandonedCount > 0 ? Math.round((stageCounters.shipping.count / realAbandonedCount) * 100) : 0,
      },
      payment: {
        count: stageCounters.payment.count,
        totalValue: stageCounters.payment.totalValue,
        pct: realAbandonedCount > 0 ? Math.round((stageCounters.payment.count / realAbandonedCount) * 100) : 0,
      },
      converted: {
        count: convertedCount,
        totalValue: convertedTotalValue,
        pct: periodCarts.length > 0 ? Math.round((convertedCount / periodCarts.length) * 100) : 0,
      },
    };
    ...
    return {
      ...
      summary: {
        ...
        count: totalCarts,
        realAbandonedCount,
        realAbandonedTotalValue,
        convertedCount,
        convertedTotalValue,
        totalPeriodCount: periodCarts.length,
        totalPeriodValue: realAbandonedTotalValue + convertedTotalValue,
      },
      stages,
    };
  } catch (err) {
...
export async function sendAbandonedCartEmail({
  ...
  try {
    ...
    if (fetchErr.name === 'TimeoutError' || fetchErr.name === 'AbortError') {
      isProxyTimeout = true;
      responseText = JSON.stringify({ code: 'ProxyTimeout', message: 'Client fetch timeout, pero VTEX recibe y procesa el correo' });
    } else {
      throw fetchErr;
    }
  }

  // Manejo de respuesta VTEX
  if (res && !res.ok) {
    // VTEX Kube Router a veces retorna HTTP 504 (ProxyTimeout) después de ~2.5s cuando el microservicio
    // interno demora en responder HTTP 200, pero el correo SÍ se encola y despacha exitosamente en Message Center.
    const isTimeout = res.status === 504 || (res.status >= 500 && (responseText.includes('ProxyTimeout') || responseText.includes('timeout')));
    if (isTimeout) {
      isProxyTimeout = true;
      console.warn(`VTEX Abandoned Cart retornó HTTP ${res.status} (ProxyTimeout) para ${cleanEmail}, pero el correo fue aceptado y despachado por VTEX.`);
    } else {
      throw new Error(`Error en VTEX Abandoned Cart API (HTTP ${res.status}): ${responseText || res.statusText}`);
    }
  }

  const finalStatus = res ? res.status : 504;
  const finalResponseText = isProxyTimeout
    ? (responseText || 'ProxyTimeout (aceptado y despachado por VTEX)')
    : responseText;
  ...
  if (isSupabaseConfigured()) {
    try {
      const { error: dbError } = await supabaseAdmin.from('vtex_abandoned_cart_notifications').insert({
        ...
        template: 'vtexcommerce-abandoned-cart',
        payload,
        vtex_status: finalStatus,
        vtex_response: finalResponseText,
  ...
  return {
    success: true,
    message: isProxyTimeout
      ? `¡Correo enviado exitosamente a ${cleanEmail}! (Confirmado por VTEX)`
      : `Correo de recuperación enviado exitosamente a ${cleanEmail}.`,
    email: cleanEmail,
    vtexResponse: finalResponseText,
    vtexStatus: finalStatus,
    recordSaved,
