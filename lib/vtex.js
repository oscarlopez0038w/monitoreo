...
export async function fetchSkuDetails(skuId) {
  ...
  while (retries > 0) {
    try {
      ...
      let productIsVisible = null;

      if (data.ProductId) {
        try {
          const productUrl = `${config.baseUrl}/api/catalog/pvt/product/${data.ProductId}`;
          const productRes = await fetch(productUrl, { method: 'GET', headers, cache: 'no-store' });
          if (productRes.ok) {
            const productData = await productRes.json();
            productIsVisible = productData.IsVisible === true;
          }
        } catch (e) {}
      }
      ...
      return {
        ...
        displayOnSite: productIsVisible ?? (data.IsVisible === true || data.DisplayOnSite === true),
...
export async function toggleSkuDisplayOnSite(skuId, displayOnSite = true) {
  ...
  const skuLookupRes = await fetch(skuLookupUrl, { headers, cache: 'no-store' });
