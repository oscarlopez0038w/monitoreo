...
export async function fetchSkuPrice(skuId, { simulateDiscount = true } = {}) {
  ...
  // finalPrice representa el precio real de checkout (con todas las promos aplicadas)
  let finalPrice = rawPrice.basePrice;
  let simPromoName = null;
  let simDiscountPct = 0;

  // SIEMPRE simular checkout para capturar promos de Rates & Benefits (incluso sobre fixed prices)
  if (simulateDiscount) {
    try {
      ...
      if (sim && sim.sellingPrice > 0) {
        ...
        // Si la simulación detecta un listPrice mayor, usarlo como referencia
        if (sim.listPrice && sim.listPrice > finalPrice) {
          finalListPrice = sim.listPrice;
        }

        // Capturar nombre de promo y % de descuento de la simulación
        if (sim.appliedPromotions && sim.appliedPromotions.length > 0) {
          simPromoName = sim.appliedPromotions.find(p => !p.includes('@price')) || sim.appliedPromotions[0];
        }
        if (sim.discountPercentage > 0) {
          simDiscountPct = sim.discountPercentage;
        }

        // Si no hay descuento detectado por Pricing API pero la simulación sí muestra uno, actualizar basePrice
        if (finalListPrice === null || finalListPrice === finalBasePrice) {
          if (sim.sellingPrice < (sim.listPrice || finalBasePrice)) {
            finalBasePrice = sim.sellingPrice;
            if (sim.listPrice) finalListPrice = sim.listPrice;
          }
        }
      }
    } catch (simErr) {
      // Conservar los valores base si la simulación falla
