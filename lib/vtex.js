...
export async function fetchSkuPrice(skuId) {
  ...
  while (retries > 0) {
    try {
      ...
      // 1. Filtrar solo precios fijos ACTIVOS hoy (Canal Principal / Trade Policy 1)
      const activeFixedPrices = (data.fixedPrices || []).filter(isFixedPriceActive);
      const fixedP = activeFixedPrices.find((f) => String(f.tradePolicyId) === '1') || activeFixedPrices[0] || null;
 ...
 */
export async function simulateSkuPrice(skuId) {
  const config = getVtexConfig();
  ...
  try {
    ...
    const listPrice = item.listPrice ? item.listPrice / 100 : null;
    const sellingPrice = item.sellingPrice ? item.sellingPrice / 100 : null;
    const priceTags = item.priceTags || [];
    ...
    return {
      ...
      sellingPrice,
      priceTags,
      appliedPromotions: priceTags.map((t) => ({ name: t.name, identifier: t.identifier, value: (t.value || 0) / 100 })),
    };
  } catch (err) {
    console.error(`Error simulando promociones para SKU ${skuId}:`, err);
    return null;
  }
}
