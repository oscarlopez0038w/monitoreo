...
export default function TransactionsTable({ transactions, onSelectTransaction, isLoading }) {
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      {/* VISTA MÓVIL (<769px) TARJETAS TÁCTILES */}
          ...
          return (
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: '0.6rem' }}>
                ...
                <code style={{ background: 'rgba(56, 189, 248, 0.12)', border: '1px solid rgba(56, 189, 248, 0.25)', color: '#38bdf8', padding: '0.2rem 0.5rem', borderRadius: '6px', fontSize: '0.78rem', fontWeight: 700 }}>
                  #{tx.key}
                </code>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem' }}>
                <div>
                  <strong style={{ color: '#ffffff', fontSize: '0.92rem', display: 'block', lineHeight: '1.2' }}>
                    {tx.client?.name || 'Cliente General'}
                  </strong>
                  <span style={{ fontSize: '0.74rem', color: 'var(--text-dim)', display: 'block', marginTop: '0.15rem' }}>
                    {tx.client?.email || 'Sin correo registrado'}
                  </span>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <strong style={{ fontSize: '1.05rem', color: '#38bdf8', fontWeight: 800 }}>
                    {tx.amount?.toLocaleString('es-NI', { minimumFractionDigits: 2 })} {tx.currency || 'NIO'}
                  </strong>
                </div>
              </div>

              {/* Método de Pago Pills & Diagnóstico */}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.5rem', paddingTop: '0.4rem', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', flexWrap: 'wrap' }}>
                  <span style={{ padding: '0.2rem 0.5rem', borderRadius: '6px', background: 'rgba(255,255,255,0.08)', fontSize: '0.74rem', color: '#f8fafc', fontWeight: 600 }}>
                    💳 {tx.payment?.cardBrand || tx.payment?.systemName || 'Tarjeta'}
                  </span>

                  <span style={{ padding: '0.15rem 0.45rem', borderRadius: '6px', fontSize: '0.72rem', fontWeight: 800, backgroundColor: tx.payment?.isFinanced ? 'rgba(168, 85, 247, 0.2)' : 'rgba(34, 197, 94, 0.15)', color: tx.payment?.isFinanced ? '#c084fc' : '#4ade80' }}>
                    {tx.payment?.isFinanced ? `⚡ Cuotas (x${tx.payment.installments})` : 'Contado'}
                  </span>
                </div>
                ...
                <button
                  ...
                  style={{ padding: '0.35rem 0.75rem', fontSize: '0.76rem', minHeight: '34px', borderRadius: '8px' }}
                >
